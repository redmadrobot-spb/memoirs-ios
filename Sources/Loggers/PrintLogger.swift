...
public struct PrintLogger: Logger {
    ...
    public func log(
        ...
        let metaDescription = meta().map { $0.isEmpty ? "" : "\($0.mapValues { $0.privateExcluded(false) })" }
        let description = [ "\(timestamp)", "\(level)", context, "\(label)", message().privateExcluded(false), metaDescription ]
            .compactMap { $0 }
            .filter { !$0.isEmpty }
            .joined(separator: " ")
        print(description)
